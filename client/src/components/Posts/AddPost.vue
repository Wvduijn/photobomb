    ...
    <v-layout row wrap>
      <v-flex xs12 sm6 offset-sm3>
        ...
        <v-form v-model="isFormValid" lazy-validation ref="form" @submit.prevent="handleAddPost">
          ...
          <v-layout row>
            <v-flex xs12>
              <v-select v-model="categories" :rules="categoriesRules" :items="['Selfie', 'Reizen', 'Fun', 'Fotografie', 'Sport', 'Foodporn']" multiple label="Categorien"></v-select>
            </v-flex>
          </v-layout>
...
export default {
  ...
  data() {
    return {
      ...
      description: "",
      titleRules: [
        title => !!title || "Title is verplicht",
        title => title.length < 20 || "Title moet minder dan 20 karakters bevatten"
      ],
      imageRules: [image => !!image || "Image is verplicht"],
      categoriesRules: [
        categories =>
          categories.length >= 1 || "Ten minste 1 categorie is verplicht"
      ],
      descRules: [
        desc => !!desc || "Beschrijving is verplicht",
        desc =>
          desc.length < 200 || "Beschrijving mag niet meer dan 200 karakters bevatten"
      ]
    };
  },
  computed: {
